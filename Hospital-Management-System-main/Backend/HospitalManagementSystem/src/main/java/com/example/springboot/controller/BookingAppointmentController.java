...
public class BookingAppointmentController {
    ...
    public ResponseEntity<String> deleteAppointmentById(@PathVariable("appointmentId") long appointmentId) {
        if (bookingAppointmentService.deleteAppointmentById(appointmentId)) {
            return new ResponseEntity<>("Appointment deleted successfully", HttpStatus.OK);
        } else {
            return new ResponseEntity<>("Appointment not found or unable to delete", HttpStatus.NOT_FOUND);
        }
    }
